...
### انزلاق التسمية عند نسخ (Duplicate) الإعلانات — مخالفة مؤكدة ومُصحَّحة
- **أين:** Meta، حساب Naqi Bahrain، حملة `2026_BA_NAQI_SOC_ING_LEAD-GEN_Summer2026`، Ad Set `ING_FltrslvrqClrSafe_BA_Summer2026`.
- **ما حدث:** إعلان تم نسخه (Duplicate) داخل نفس الـ Ad Set، فأضافت منصة Meta تلقائياً لاحقة "– Copy 2" لاسم الإعلان الجديد، بينما بقي الرابط (source/utm_source/utm_campaign) محتفظاً بالاسم الأصلي **بدون** اللاحقة. النتيجة: اسم الإعلان في تقارير Meta ≠ القيمة الفعلية في GA/UTM لنفس الإعلان.
  - قبل التصحيح: الاسم `ING_FltrslvrqClrSafe_pkg_IMG_BA_Summer2026_T2 – Copy 2` ↔ الرابط بقيمة `...T2` (بدون Copy 2).
  - بعد التصحيح (2026-08-09): الاسم أصبح `ING_FltrslvrqClrSafe_pkg_IMG_BA_Summer2026_T2` — مطابق تماماً لقيم source/utm_source/utm_campaign في الرابط. لم يتم تعديل أي شيء آخر (لا الميزانية، لا الاستهداف، لا الرابط نفسه، لا الحالة — بقي ACTIVE طوال الوقت).
- **الدرس:** أي نسخ (Duplicate) لإعلان — سواء يدوي أو عبر أداة — يجب أن يُتبع فوراً بفحص اسم الإعلان الناتج مقابل رابطه (خطوة 6 في القسم 7 أدناه)؛ منصة Meta تضيف لاحقات تلقائية (Copy, Copy 2…) لا تعرفها أدوات الـ UTM.
- **توصية عملية:** بعد أي Duplicate على Meta، غيّر اسم النسخة الجديدة فوراً لاسم مطابق لصيغة القسم 3 (مع رفع رقم النسخة V/T)، **قبل** تفعيلها أو حتى لو بقيت PAUSED — لا تعتمد على الاسم التلقائي أبداً.
